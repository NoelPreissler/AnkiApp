...
// Lokale Standard-Lektionen
const STATISCHE_LEKTIONEN = {
...
export default function App() {
  ...
  // Kombinierte Lektionsliste aus lokalen JSONs + DB-Sätzen
  const [alleLektionen, setAlleLektionen] = useState({ ...STATISCHE_LEKTIONEN });
  const [selectedLektion, setSelectedLektion] = useState(Object.keys(STATISCHE_LEKTIONEN)[0]);
  ...
  // User-Session und lokales Deck beim Start prüfen
  useEffect(() => {
  ...
  // Dynamisch die Vokabelsätze des Users aus der DB laden und anhängen
  useEffect(() => {
    if (user) {
      fetch(`http://193.197.231.68:5000/api/saetze/${user.id}`)
        .then(res => res.json())
        .then(saetze => {
          const neueListe = { ...STATISCHE_LEKTIONEN };
          // Jeden Satz aus der DB als Platzhalter-Objekt markieren
          saetze.forEach(satz => {
            neueListe[`📁 DB: ${satz.name}`] = { isDbSatz: true, id: satz.id };
          });
          setAlleLektionen(neueListe);
        })
        .catch(err => console.error("Fehler beim Laden der DB-Sätze", err));
    } else {
      setAlleLektionen({ ...STATISCHE_LEKTIONEN });
      setSelectedLektion(Object.keys(STATISCHE_LEKTIONEN)[0]);
    }
  }, [user, currentView]);
  ...
  // Startet das Quiz und unterscheidet zwischen lokalen JSONs und DB-Anfragen
  const handleStartQuiz = async () => {
    const gewaehlteLektion = alleLektionen[selectedLektion];
    if (!gewaehlteLektion) return;

    let roheVokabeln = [];

    // Fall 1: Es ist ein Satz aus der PostgreSQL Datenbank
    if (gewaehlteLektion.isDbSatz) {
      try {
        const response = await fetch(`http://193.197.231.68:5000/api/vokabeln/satz/${gewaehlteLektion.id}`);
        const dbData = await response.json();
        // Mappe DB-Spalten (deutsch, zielsprache) auf die UI-Struktur (vorn, hinten)
        roheVokabeln = dbData.map(v => ({ vorn: v.deutsch, hinten: v.zielsprache }));
      } catch (err) {
        alert("Fehler beim Laden der Vokabeln aus der Datenbank.");
        return;
      }
    } else {
      // Fall 2: Es ist ein lokales importiertes JSON
      roheVokabeln = gewaehlteLektion;
    }

    if (roheVokabeln.length === 0) {
      alert("Dieser Vokabelsatz enthält keine Karten!");
      return;
    }

    // Normalisierung der Sprachrichtung
    const normalizedDeck = roheVokabeln.map(item => {
          ...
          !quizStarted ? (
            <div style={styles.container}>
              ...
              <div style={styles.selectBox}>
                <label style={{ fontWeight: 'bold', color: '#475569' }}>Wähle eine Lektion oder ein DB-Deck:</label>
                <select value={selectedLektion} onChange={(e) => setSelectedLektion(e.target.value)} style={styles.select}>
                  {Object.keys(alleLektionen).map(name => <option key={name} value={name}>{name}</option>)}
                </select>
...
const styles = {
  ...
  select: { width: '100%', padding: '12px', borderRadius: '6px', border: '2px solid #cbd5e1', fontSize: '16px', backgroundColor: '#fff' },
  ...
  card: { minHeight: '120px', border: '2px solid', borderRadius: '10px', padding: '30px 20px', margin: '20px 0', fontSize: '24px', fontWeight: 'bold', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', userSelect: 'none' },
